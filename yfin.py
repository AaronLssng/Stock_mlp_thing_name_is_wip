...
def fetch_index_data(index_symbol="^GSPC", start_date=None, end_date=None, years=10):
    ...
    log(
        f"Fetching {index_symbol} from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"
    )
...
def fetch_stock_data(symbols, start_date=None, end_date=None, years=10):
    ...
    log(
        f"Fetching data for {len(symbols)} stocks from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"
    )
    ...
    for i in range(0, len(symbols), BATCH_SIZE):
        ...
        if hasattr(batch_data.columns, "levels"):
            ...
            volumes = (
                batch_data["Volume"]
                if "Volume" in batch_data.columns.levels[0]
                else None
            )
        else:
...
def fetch_individual_stock(symbol, years=10, timeframe="1d"):
    ...
    try:
        ...
        if isinstance(data.columns, pd.MultiIndex):
            print(f"MultiIndex detected for {symbol}, extracting...")
            # Try to get just this symbol's data
            if symbol in data.columns.get_level_values(1):
                data = data.xs(symbol, level=1, axis=1)
            else:
                # Take the first level (usually 'Close', 'Open', etc.)
        ...
        for col in expected_cols:
            if col not in data.columns:
                print(f"Warning: {col} not found in data for {symbol}")

        print(f"Fetched {symbol}: {len(data)} bars, columns: {data.columns.tolist()}")
        return data
            ...
            if isinstance(daily_prices, pd.Series):
                ...
                if len(clean_prices) > 0:
                    ...
                    if idx > 0:
                        ...
                        if len(common) > 0:
                            returns = (
                                clean_prices[common] / prev_prices[common] - 1
                            ) * 100
...
def fetch_summary_data(index_type="sp500", years=10, save_csv=None):
    ...
    index_symbols = {"sp500": "^GSPC", "dow": "^DJI", "nasdaq100": "^NDX"}

    index_names = {"sp500": "SP500", "dow": "DOW", "nasdaq100": "NAS100"}
    ...
    index_data = fetch_index_data(
        index_symbols.get(index_type, "^GSPC"), start_date, end_date
    )
...
def save_stock_to_binary(symbol, df, timeframe="1d"):
    ...
    # Find the date column name (could be 'Date' or 'index' or whatever)
    date_col = df_reset.columns[0]  # First column after reset is always the index
    print(f"Date column name: {date_col}")

    # Build DataFrame manually
    timestamps = pd.to_datetime(df_reset[date_col]).astype(np.int64) // 10**9

    # Get each column as 1D array
    df_storage = pd.DataFrame(
        {
            "timestamp": timestamps,
            "open": df_reset["Open"].values.astype(np.float32),
            "high": df_reset["High"].values.astype(np.float32),
            "low": df_reset["Low"].values.astype(np.float32),
            "close": df_reset["Close"].values.astype(np.float32),
            "volume": df_reset["Volume"].values.astype(np.float64),
        }
    )
...
if __name__ == "__main__":
    ...
    print(
        "  from yfin import fetch_and_save_stock, load_stock_from_binary, fetch_sp500_summary"
    )
    print("\nExample usage:")
